...
both_engines!(
    the_catalog_lists_tables_and_views,
...
both_engines!(
    values_decode_to_the_right_variants,
    |fx: Fixture| async move {
        let rows = fx
            .table_rows(
                &fx.people(),
...
both_engines!(
    an_empty_result_still_has_its_headers,
    |fx: Fixture| async move {
...
both_engines!(
    a_hostile_table_name_survives_quoting,
    |fx: Fixture| async move {
        ...
        match error {
            DriverError::Query { message, .. } => {
                assert!(
                    message
                        .to_lowercase()
                        .contains("definitely_not_a_table_xyz"),
...
both_engines!(filters_and_updates_round_trip, |fx: Fixture| async move {
    let table = fx.people();
    ...
    let matched = fx.row_count(&table, where_like).await.expect("like count");
...
both_engines!(
    deleting_a_missing_row_affects_nothing,
    |fx: Fixture| async move {
...
both_engines!(
    generated_truncate_and_drop_are_accepted,
    |fx: Fixture| async move {
        ...
        fx.execute(&dbui_driver::drop_sql(
            fx.driver(),
            &table,
            TableKind::Table,
        ))
        .await
        .expect("drop");
        assert!(fx.row_count(&table, "").await.is_err(), "the table is gone");
...
both_engines!(
    paging_in_key_order_sees_every_row_once,
    |fx: Fixture| async move {
...
// to be holding.
both_engines!(
    a_sort_is_applied_across_the_whole_table,
    |fx: Fixture| async move {
        ...
        let first = fx
            ...
            .expect("first row");
        assert_eq!(
            first.rows[0].get(1).map(|v| v.to_text()),
            Some("Grace".into())
        );
    }
);
...
both_engines!(
    an_insert_commits_with_the_rest_of_the_batch,
    |fx: Fixture| async move {
        ...
        assert_eq!(rows.rows.len(), 1);
        assert_eq!(
            rows.rows[0].get(1).map(|v| v.to_text()),
            Some("Katherine".into())
        );
        assert!(
...
both_engines!(
    a_failing_insert_rolls_the_batch_back,
    |fx: Fixture| async move {
        use dbui_driver::{RowBatch, RowInsert, RowUpdate};
...
both_engines!(
    foreign_keys_are_read_for_single_column_references,
    |fx: Fixture| async move {
...
both_engines!(
    a_composite_foreign_key_is_not_reported,
    |fx: Fixture| async move {
...
// convention.
both_engines!(
    a_read_only_connection_is_refused_by_the_server,
    |fx: Fixture| async move {
        let table = fx.people();
        let quoted = table.quoted(fx.driver());
        ...
        let err = ro
            .execute(&format!(
                "UPDATE {quoted} SET nickname = 'nope' WHERE id = 1"
            ))
            .await
